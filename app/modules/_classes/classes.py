...
class Classes:
    ...
    def to_json(self):
        from app.modules.student._student import (
            Student,
        ...
        json_object = {
            ...
            "teacher_info": {
                ...
                if self.syllabus is not None and len(self.syllabus) > 1
                else "",
                "syllabus_id": self.syllabus[0]
                if self.syllabus is not None and len(self.syllabus) > 0
                else "",
            },
            "immutable_info": {
                "alias": self.department
                + "_"
                + str(self.number)
                + "_"
                + re.sub(r"[^a-zA-Z]+", "", self.name.lower()),
    ...
    def add_assignment(self, assignment: Assignment):
        try:
            ...
            db.classes.find_one_and_update(
                {"_id": self.ID}, {"$push": {"assignments": dictionary}}
            )
        except BaseException as e:
    ...
    def update_syllabus(self, syllabus: tuple):
        try:
            if syllabus[1] != "":
                ...
                db.classes.find_one_and_update(
                    {"_id": self.ID}, {"$set": {"syllabus": self.syllabus}}
                )
        except BaseException as e:
