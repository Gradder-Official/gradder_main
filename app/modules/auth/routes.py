...
def change_password():
    ...
    curr_user = eval(current_user.USERTYPE).get_by_id(
        current_user.ID
    )  #  Pull the user from the DB, as the current user is not updated
    ...
    if form.validate_on_submit():
        if curr_user.verify_secret_question(form.secret_question.data.lower()):
            ...
            if curr_user.add():
                ...
                logger.error(
                    "Unknown error while changing the password: {} {} - ACCESS: {}".format(
                        curr_user.first_name, curr_user.last_name, curr_user.USERTYPE
                    )
                )
...
def change_secret_question():
    ...
    if form.validate_on_submit():
        if curr_user.verify_password(form.password.data):
            curr_user.set_secret_question(
                form.new_secret_question.data, form.new_secret_answer.data.lower()
            )
