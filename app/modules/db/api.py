...
class DB:
    ...
    def get_user_by_name(self, usertype:User, first_name:str, last_name:str):
        if first_name and last_name:
            received_user = eval(f'self.collection_{type(usertype).__name__.lower()+"s"}.where(u"last_name", u"==", {last_name}).where(u"first_name", u"==", {first_name})')
            if received_user:
    ...
    def delete_user(self, user:User):
        eval(f'self.collection_{type(user).__name__.lower()+"s"}.document(str(user.ID)).delete()')
    ...
    def add_subscriber(self, subscriber:Subscriber):
        try:
            self.collection_subscribers.document(str(subscriber.ID)).set(subscriber.to_dict())
            return True
        except BaseException:
            return False
